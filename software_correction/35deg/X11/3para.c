...
main()
{
 ...
 // float a,b,c,d,e,f;
  ...
  static float sp[3]={0.0,0.0,0.0};
FILE *fp;
  fp = fopen("Y1fp_data.dat","r");
  while((ff=fscanf(fp,"%f %f %f",&p,&t,&x)) != EOF){ //p t x : Y1fp Yfp THfp
    a=1;
    b=t;
    c=x*t;
    ms[0][0]=ms[0][0]+a*a;
  ...
  for(i=0;i<=2;i++){
    printf("%f  %f  %f  %f\n",
	   ms[i][0],ms[i][1],ms[i][2],sp[i]);
  }
  printf("-------------------\n");
  for(k=0;k<=2;k++){             /* unit line */
    uu=ms[k][k];
    for(j=0;j<=2;j++){             /* make unit */
      ms[k][j]=ms[k][j]/uu;
    }
    sp[k]=sp[k]/uu;
    /*
      printf("%f  ",uu);
    */
    for(i=0;i<=2;i++){           /* other lines */
      if(i!=k){
	uu=ms[i][k];
	for(j=0;j<=2;j++){         /* subtractions */
