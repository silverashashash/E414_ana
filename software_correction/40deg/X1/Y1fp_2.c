...
//for Y1fp =  A + B*Yfp + C*Yfp*THfp + D*THfp
...
main()
{
 ...
 // float a,b,c,d,e,f;
  ...
  static float sp[4]={0.0,0.0,0.0,0.0};
FILE *fp;
  ...
  printf("-------------------\n");
  for(k=0;k<=3;k++){             /* unit line */
    uu=ms[k][k];
    for(j=0;j<=3;j++){             /* make unit */
      ms[k][j]=ms[k][j]/uu;
    }
    sp[k]=sp[k]/uu;
    /*
      printf("%f  ",uu);
    */
    for(i=0;i<=3;i++){           /* other lines */
      if(i!=k){
	uu=ms[i][k];
	for(j=0;j<=3;j++){         /* subtractions */
